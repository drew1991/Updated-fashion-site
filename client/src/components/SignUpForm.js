...
function SignUpForm({ onLogin }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="signup">
      ...
      <FormField>
        <InputLabel id="select-label"><span style={{color:'lightblue'}} >Are you a seller</span></InputLabel>
        <div>
            <input
            type="radio"
            value={sellerTrue}
        ...
        <div>
            ...
            /> <span style={{color:'lightblue'}} >no</span>
        </div>
      </FormField>
      <FormField>
        <button type="submit" className="button-89" style={{borderRadius:'20px'}}>{isLoading ? "Loading..." : "Sign Up"}</button>
      </FormField>
      <FormField>
        {errors.map((err) => (
